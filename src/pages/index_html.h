...
// Generado por tools/build_index_html.py desde src/pages/index.html
// NO editar a mano: editar index.html y regenerar.
...
<body>
  ...
  <script>
    ...
    const Api = {
      ...
      // ejecutar, caudalBomba, caudalManual). Devuelve {httpOk, ok, ...}.
      ...
      // GET /programas → { caudalBomba, caudalManual, programas:[...] }.
      // Sincroniza el caudal de bomba/manual global y devuelve los programas
      // en el formato interno de la UI.
      async getProgramas() {
      ...
      setCaudalManual(arr) { return this._postConfig({ caudalManual: arr }); },
      ...
      // GET /control?type=sector&id=N&state=0|1 → {httpOk, status, ok, error}.
      // El firmware responde 409 si no hay caudal libre para encender el sector.
    ...
    function renderStatus() {
      ...
      const activeProg = programs.find((p) => p.id === liveState.programaActivo);
    ...
    async function toggleSectorManual(sectorId) {
      ...
      // Chequeo local de caudal como feedback inmediato. La verdad la tiene el
      // firmware (que también valida y responde 409), pero así evitamos un
      // round-trip cuando ya sabemos que no hay caudal libre.
      if (turnOn) {
      ...
      try {
        const res = await Api.controlSector(sectorId, turnOn ? 1 : 0);
        if (!res.httpOk) {
          toast(res.status === 409
            ? "Sin caudal libre en la bomba para ese sector"
            : (res.error || "No se pudo cambiar el sector"), true);
          await refreshStatus();
          return;
        }
      } catch (e) {
    ...
    async function saveProgram() {
      ...
      // El firmware asigna el id y persiste; acá solo enviamos el programa.
      try {
    ...
    async function saveSystemConfig() {
      ...
      const manual = Array.from({ length: NUM_SECTORS }, (_, i) => {
        const raw = parseInt(($(`#manualFlow${i + 1}`) || {}).value, 10) || MANUAL_SECTOR_FLOW_DEFAULT;
        return Math.max(1, Math.min(val, raw));
      });

      // El firmware persiste y hace cumplir el límite: dos acciones POST
      // (caudalBomba y caudalManual). El estado real se relee con refreshStatus.
      try {
        const r1 = await Api.setCaudalBomba(val);
        if (!r1.httpOk || r1.ok === false) {
          toast(r1.error || "No se pudo guardar el caudal de la bomba", true);
          return;
        }
        const r2 = await Api.setCaudalManual(manual);
        if (!r2.httpOk || r2.ok === false) {
          toast(r2.error || "No se pudo guardar el caudal manual", true);
          return;
        }
      } catch (e) {
        toast("Error de conexión", true);
      ...
      systemConfig.caudalManual = manual;
      ...
      await refreshStatus(); // refrescar barra de caudal y estado de los sectores
    ...
    // --- Polling: leer el estado real del firmware (el motor vive en el ESP32) ---
